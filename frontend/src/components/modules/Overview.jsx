...
import {
    ...
    Calendar, StickyNote, Zap, Award, Star,
    Activity, CheckCircle2, Circle, Layers
} from 'lucide-react';
...
const StatCard = ({ icon, label, value, color, sub }) => (
    ...
    >
        <div style={{ position: 'absolute', top: '-24px', right: '-24px', width: '80px', height: '80px', borderRadius: '50%', backgroundColor: color, opacity: 0.07, pointerEvents: 'none' }} />
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: 500 }}>{label}</span>
            <div style={{ width: '32px', height: '32px', borderRadius: '8px', backgroundColor: color + '22', color, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {icon}
            </div>
        </div>
...
const ProgressRing = ({ pct, color, size = 80 }) => {
    ...
    return (
        <svg width={size} height={size}>
            <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="rgba(255,255,255,0.25)" strokeWidth={8} />
            <circle
            ...
            <text x="50%" y="50%" dominantBaseline="middle" textAnchor="middle"
                style={{ fontSize: '1rem', fontWeight: 700, fill: 'white' }}>
                {pct}%
            </text>
        </svg>
    );
...
// ─── Main Overview ───────────────────────────────────────────────────────
const Overview = () => {
    const { subjects, tasks, pomodoroSessions, schedule, notes, flashcards } = useData();
    const { user } = useAuth();
    ...
    const studyMins = pomodoroSessions.reduce((acc, s) => acc + s.duration, 0);
    const studyHours = studyMins >= 60 ? Math.round(studyMins / 60) + 'h' : studyMins + 'm';
    ...
    const stats = [
        ...
        { label: 'Pending Tasks', value: pendingTasks, color: 'var(--warning-color)', icon: <Target size={16} />, sub: `${completedTasks} completed` },
        { label: 'Study Time', value: studyHours, color: 'var(--purple-color)', icon: <Clock size={16} />, sub: 'total focus time' },
        { label: 'Study Streak', value: `${streak}d`, color: 'var(--danger-color)', icon: <Flame size={16} />, sub: 'consecutive days' },
    ];
    ...
    return (
        ...
        >
            ...
            >
                ...
                <div>
                    ...
                    <p style={{ opacity: 0.9, fontSize: '0.875rem', marginTop: '0.5rem', maxWidth: '340px' }}>
                        You have <strong>{pendingTasks}</strong> task{pendingTasks !== 1 ? 's' : ''} pending and <strong>{todaySchedule.length}</strong> session{todaySchedule.length !== 1 ? 's' : ''} today.
                    </p>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem' }}>
                ...
                {/* Today's Schedule */}
                <motion.div variants={itemVariants} initial="hidden" animate="visible" className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    ...
                    {todaySchedule.length === 0 ? (
                        <div style={{ textAlign: 'center', color: 'var(--text-secondary)', padding: '1.5rem 0', fontSize: '0.875rem' }}>
                            <Calendar size={32} style={{ opacity: 0.3, margin: '0 auto 0.5rem', display: 'block' }} />
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                            {todaySchedule.map((s, i) => (
                                <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                                    style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem', backgroundColor: 'var(--bg-color)', borderRadius: '0.75rem', border: '1px solid var(--border-color)' }}>
                                    <div style={{ width: '4px', height: '36px', borderRadius: '2px', backgroundColor: 'var(--primary-color)', flexShrink: 0 }} />
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ fontWeight: 600, fontSize: '0.875rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.subject}</div>
                ...
                <motion.div variants={itemVariants} initial="hidden" animate="visible" className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    ...
                    {urgentTasks.length === 0 ? (
                        <div style={{ textAlign: 'center', color: 'var(--text-secondary)', padding: '1.5rem 0', fontSize: '0.875rem' }}>
                            <CheckCircle2 size={32} color="var(--success-color)" style={{ margin: '0 auto 0.5rem', display: 'block', opacity: 0.6 }} />
                            <div>All caught up. No pending tasks found.</div>
                        </div>
                    ) : (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                            {urgentTasks.map((t, i) => {
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem' }}>
                ...
                <motion.div variants={itemVariants} initial="hidden" animate="visible" className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    ...
                    {subjects.length === 0 ? (
                        <div style={{ textAlign: 'center', color: 'var(--text-secondary)', padding: '1.5rem 0', fontSize: '0.875rem' }}>
                            <BookOpen size={32} style={{ opacity: 0.3, margin: '0 auto 0.5rem', display: 'block' }} />
                                ...
                                return (
                                    <div key={sub.id || i}>
                                        ...
                                        <div style={{ height: '7px', borderRadius: '4px', backgroundColor: 'var(--border-color)', overflow: 'hidden' }}>
                                            <motion.div initial={{ width: 0 }} animate={{ width: `${subPct}%` }} transition={{ duration: 0.8, delay: i * 0.1 }}
                ...
                <motion.div variants={itemVariants} initial="hidden" animate="visible" className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    ...
                    {recentNotes.length === 0 ? (
                        <div style={{ textAlign: 'center', color: 'var(--text-secondary)', padding: '1.5rem 0', fontSize: '0.875rem' }}>
                            <StickyNote size={32} style={{ opacity: 0.3, margin: '0 auto 0.5rem', display: 'block' }} />
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                            {recentNotes.map((note, i) => {
                                ...
                                return (
                                    <div key={note.id || i} style={{ padding: '0.75rem 1rem', borderRadius: '0.75rem', backgroundColor: nc + '33', borderLeft: `3px solid ${nc}88`, border: `1px solid ${nc}55` }}>
                ...
                <motion.div variants={itemVariants} initial="hidden" animate="visible" className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                        {[
                            ...
                            { icon: <Layers size={18} />, label: 'Card Master', sub: `${totalFlashcards} flashcards created`, unlocked: totalFlashcards >= 5, color: '#06b6d4' },
                        ].map((ach, i) => (
                            <div key={i} style={{
                                ...
                                opacity: ach.unlocked ? 1 : 0.45, transition: 'all 0.2s'
                            }}>
                                <div style={{ width: '32px', height: '32px', borderRadius: '8px', backgroundColor: ach.unlocked ? ach.color + '25' : 'var(--border-color)', color: ach.unlocked ? ach.color : 'var(--text-secondary)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                    {ach.icon}
                                </div>
                                <div>
                                ...
                                {ach.unlocked && <div style={{ marginLeft: 'auto', color: ach.color, fontSize: '0.75rem', fontWeight: 800 }}>+</div>}
                            </div>
                        ))}
                    </div>
                </motion.div>
